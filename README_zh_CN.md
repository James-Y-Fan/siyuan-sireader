...
### 🔊 TTS 语音朗读
- **多语音支持** - 在线语音（Edge TTS）和本地语音（浏览器 TTS）
- **智能播放** - 选中文本循环播放、从选中段落播放、从当前页播放
- **精准高亮** - PDF 朗读时精确高亮当前文本，EPUB 自动滚动到当前段落
- **暂停继续** - 播放中点击暂停，再次点击继续播放
- **翻页停止** - 翻页、跳转等操作自动停止播放，避免位置错乱
...
### 🔊 TTS 语音朗读系统
完整的语音朗读功能，支持多种智能播放场景：

**语音选项：**
- **在线语音** - Edge TTS 提供 100+ 种多语言语音
- **本地语音** - 浏览器内置 TTS（无需网络）
- **语音管理** - 收藏常用语音，快速切换
- **语速控制** - 可调节播放速度（0.5x - 2.0x）

**智能播放场景：**
- **选中文本循环播放** - 选中文本 → 点击选择菜单朗读按钮 → 循环播放选中内容，取消选中自动停止
- **从选中段落播放** - 选中文本 → 点击底部播放按钮 → 从选中段落开始往下连续播放
- **从当前页播放** - 无选中 → 点击底部播放按钮 → 从当前页开始往下连续播放
- **暂停/继续播放** - 播放中点击底部播放按钮暂停，再次点击继续播放
- **翻页自动停止** - 翻页、跳转等操作自动停止播放，避免位置错乱
- **新位置重新播放** - 翻页后点击播放从新页面开始，选中新文本后从新位置开始

**阅读功能：**
- **精准高亮** - PDF 朗读时精确高亮当前文本，自动翻页
- **自动滚动** - EPUB 自动滚动到当前段落
- **文本提取** - 智能识别 EPUB 和 PDF 的段落
- **预加载机制** - 提前加载 3 个段落，实现无缝播放
- **设置选项** - 文本高亮开关、自动翻页、播放速度
